...
class MassFamilyProcessorGUI:
    ...
    def load_config(self):
        ...
        try:
            ...
            self.log_message(f"⚠️ Ошибка загрузки конфигурации: {e}")
            return self.get_default_config()
    ...
    def on_closing(self):
        ...
        if self.is_processing:
            response = messagebox.askyesno(
                ...
                "Идет обработка семей. Вы уверены, что хотите выйти?"
            )
            if not response:
                return
    ...
    def process_single_family(self, family_data, family_number):
        """Обработка одной семьи (устаревший метод - используйте process_single_family_with_retry)"""
        try:
            # Создаем или переиспользуем драйвер
            if not self.driver:
                self.auto_filler = AutoFormFillerMass(self)
                if not self.auto_filler._setup_driver():
                    self.log_message("❌ Не удалось настроить драйвер")
            ...
            self.log_message(f"❌ Ошибка в process_single_family: {str(e)}")
            return False
    ...
    def run(self):
        ...
        try:
            ...
            self.log_message(f"❌ Критическая ошибка приложения: {e}")
    ...
    def _fill_form(self, add_info_text, category, housing_info, adpi_data):
        ...
        try:
            ...
            try:
                ...
                self.log("✅ Чекбоксы отмечены через JavaScript")
            except Exception as e:
                self.log(f"⚠️ Не удалось отметить чекбоксы через JavaScript: {e}")
