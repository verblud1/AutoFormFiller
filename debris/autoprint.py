...
def create_printed_directory(folder_path):
    """Создает папку для сохранения напечатанных файлов"""
    printed_dir = os.path.join(folder_path, "printed")
    
    # Если папка уже существует, создаем новую с номером
    if os.path.exists(printed_dir):
        counter = 1
        while os.path.exists(f"{printed_dir}_{counter}"):
            counter += 1
        printed_dir = f"{printed_dir}_{counter}"
    
    os.makedirs(printed_dir, exist_ok=True)
    print(f"Создана папка для сохранения PDF: {printed_dir}")
...
def save_to_printed_directory(pdf_file, printed_dir, page_number, side):
    ...
    try:
        ...
        filename = f"{timestamp}_page_{page_number:03d}_{side}.pdf"
...
def main():
    ...
    # Создаем папку для сохранения напечатанных файлов
    printed_dir = create_printed_directory(folder_path)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i in range(0, total_files, 4):
            ...
            if front_files:
                ...
                # Сохраняем копию в папку printed
...
def create_readme_file(printed_dir, total_files, total_pages):
    ...
    with open(readme_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"Дата печати: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        ...
        f.write(f"Папка с исходными изображениями: {os.path.dirname(os.path.dirname(printed_dir))}\n\n")
        ...
        f.write("Формат: ГГГГММДД_ЧЧММСС_page_XXX_front/back.pdf\n")
        f.write("Пример: 20241203_143022_page_001_front.pdf\n\n")
